...
function loadVIP() {
    if (!fs.existsSync("./datab/vips.json")) fs.writeFileSync("./data/vips.json", JSON.stringify({}));
    return JSON.parse(fs.readFileSync("./datab/vips.json"));
}

function saveVIP(data) {
    fs.writeFileSync("./datab/vips.json", JSON.stringify(data, null, 2));
}
