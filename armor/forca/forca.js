...
const usuariosForcaPath = `./armor/forca/usuarios.json`
...
const existIdForcaGameInUsu = (sender, from) => {
     AB = usuarios_forca.map(i => i.id).indexOf(sender)
     data = JSON.parse(fs.readFileSync(`./armor/forca/${from}.json`))
     nmr = 0
...
const forcaWordPath = `./armor/forca/palavras.json`
